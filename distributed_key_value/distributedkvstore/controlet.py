        ...
        while True:
            try:
                ...
                if request[:3] == "set":
                    ...
                    threading.Thread(target=self.__setKey, args=( conn, split_resp[1], split_resp[2].split('\r\n')[0],)).start()
                elif request[:3] == "get":
                    split_resp = request.split(" ")
                    print("get key: ",split_resp[1])
                    threading.Thread(target=self.__getKey, args=(conn, split_resp[1].split('\r\n')[0],)).start()
                else:
